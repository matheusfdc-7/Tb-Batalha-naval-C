...
int logicaCPU(char tabuleiro[TAMA][TAMA]){
    ...
    int memoria[64][2]; // 64 é numero total de espaços, coluna 0 é x, coluna 2 é y
    ...
    int x, y;
    int chutes = 0;
    int errou = 0; // vai contar a quantidade de erros seguidos
    
    // ataca aleatoriamente
    if( contAcertos == 0 && errou >= 4){ // só vai entrar aqui se ainda não tiveram acertos ou se houve erros demais
        ...
        if( atacar(tabuleiro, x, y ) == 1 ){
            ...
            errou = 0; // atualiza variavel para seguir com os ataques inteligentes
    ...
    do {
        ...
        if( atacar(tabuleiro, x, y) == 1 ){
            ...
            errou = 0; // zera a variavel para o proximo uso
            ...
            errou++; //conta quantos erros seguidos teve
